...
data NNfactors = NNfactors
  { -- | beta coefficient for sigmoid function
    beta :: Float,
    -- | number of additional hidden layer neurons
    addHidNeuNumber :: Int,
    -- | maximal value of an additional connection weight
    addWeightLimit :: Float,
    -- | bias for additional neurons
    addNeuronsBias :: Float,
    -- | weight W factor (added value)
    weightFactor :: Float,
    -- | A_min factor (added value)
    aminFactor :: Float
  }
  deriving (Show, Read)
...
nnToPythonString (NN iL hL oL rL ihC hoC rC) =
  "{\"inpLayer\" = "
    ++ neuronsToPythonString iL
    ++ ", "
    ++ "\"hidLayer\" = "
    ++ neuronsToPythonString hL
    ++ ", "
    ++ "\"outLayer\" = "
    ++ neuronsToPythonString oL
    ++ ", "
    ++ "\"recLayer\" = "
    ++ neuronsToPythonString rL
    ++ ", "
    ++ "\"inpToHidConnections\" = "
    ++ connectionsToPythonString ihC
    ++ ", "
    ++ "\"hidToOutConnections\" = "
    ++ connectionsToPythonString hoC
    ++ ", "
    ++ "\"recConnections\" = "
    ++ connectionsToPythonString rC
    ++ "}"
